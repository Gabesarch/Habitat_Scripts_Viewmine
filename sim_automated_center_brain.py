...
class AutomatedMultiview():
    def __init__(self):   
        ...
        self.ignore_classes = ['base-cabinet','beam','blanket','blinds','cloth','clothing','coaster','comforter','curtain','ceiling','countertop','floor','handrail','mat','paper-towel','pillar','pipe','scarf','shower-stall','switch','tissue-paper','towel','vent','wall','wall-plug','window','rug','logo','set-of-clothing']
    ...
    def run_episodes(self):
        ...
        for episode in range(self.num_episodes):
            ...
            self.basepath = f"/home/nel/gsarch/data/brain/{mapname}_{episode}"
    ...
    def save_datapoint(self, agent, observations, data_path, viewnum, mainobj, flat_view):
        if self.verbose:
        ...
        # st()
        mainobj_id = mainobj.id
        #for obj_id in obj_ids:
        main_id = int(mainobj_id[1:])

        #main_id = int(mainobj.category.index())
        ...
        #     # semantic = observations["semantic_sensor"]
        #     st()
        #     im2[semantic == main_id] = [1, 0, 1]
        #     plt.imshow(im2)
        #     plt.show()

        
        for obj_id in [main_id]: # only save main object
            ...
            if class_name not in self.ignore_classes:
                if self.verbose: 
                    print("Saved class name is : ", class_name)
    ...
    def run(self):
        ...
        for obj in objects:
            ...
            if len(episodes) >= self.num_views:
                ...
                viewnum = 0
                for obs in flat_obs:
                    self.save_datapoint(self.agent, obs, data_path, viewnum, obj, True)
                    viewnum += 1
            else:
