...
class AutomatedMultiview():
    def __init__(self):
        ...
        self.visualize_maskrcnn = False
        self.visualize_obj = True
        self.verbose = False
        ...
        self.num_spawns_per_episode = 25 #20 #20
        self.turn_angle = 60.0
        self.radius_remove = 0.3
        ...
        for obj_id in obj_ids:
            ...
            if class_name in self.include_classes:
                ...
                obj_data = {'instance_id': obj_id, 'category_id': obj_instance.category.index(), 'category_name': obj_instance.category.name(),
                                 'bbox_center': obj_instance.obb.to_aabb().center, 'bbox_size': obj_instance.obb.to_aabb().sizes,
    ...
    def run(self):
        ...
        for i in range(object_centers.shape[0]-1):
            dists = np.sqrt(np.sum((object_centers[i, :] - object_centers[i+1:])**2, axis=1))
        ...
        if self.visualize_obj:
            ...
            plt.plot(object_centers_unique[:,2], object_centers_unique[:,0], 'x', color = 'green')

            plt.figure(2)
            x_sample = self.nav_pts[:,0]
            z_sample = self.nav_pts[:,2]
            plt.plot(z_sample, x_sample, 'o', color = 'red')
            plt.plot(object_centers[:,2], object_centers[:,0], 'x', color = 'green')
            plt.show()
        ...
        for obj_idx in range(object_centers_unique.shape[0]):
            ...
            if len(episodes) >= self.num_views:
                print(f'num episodes: {len(episodes)}')
                data_folder = 'obj' + str(obj_idx) #obj.category.name() + '_' + obj.id
                ...
                viewnum = 0
                for obs in flat_obs:
                    self.save_datapoint(self.agent, obs, data_path, viewnum, obj_idx, True)
                    viewnum += 1
            else:
            ...
            for angle in angles: #b_inds_notempty:
                ...
                for segs in range(len(pred_masks)):
                    if pred_classes[segs].item() in maskrcnn_to_catname:
                        if pred_scores[segs] >= 0.90:
                            ...
                            cat_ids_objects.append(maskrcnn_to_catname[int(pred_classes[segs])])
                ...
                if not obj_masks:
                    ...
                    if self.visualize_maskrcnn:
                        ...
                        z_sample = self.nav_pts[:,2]
                        plt.plot(z_sample, x_sample, 'o', color = 'red')
                        plt.plot(nav_points[:,2], nav_points[:,0], 'o', color = 'green')
                        plt.plot(agent_state.position[2], agent_state.position[0], 'x', 'blue')
                        plt.show()
