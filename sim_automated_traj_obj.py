...
class AutomatedMultiview():
    def __init__(self):   
        ...
        self.makevideo = True
        self.min_orbslam_views = 90
        self.orbslam_path = '/home/nel/ORB_SLAM2/Examples/RGB-D/replica'
        self.ORBSLAM_Cam_Traj = '/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/CameraTrajectory.txt'
        self.ORBSLAM_Cam_Keypoints = '/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/KeyFrameTrajectory.txt'
        # st()
        ...
        self.include_classes = ['bed'] #['chair', 'bed', 'toilet', 'sofa', 'indoor-plant', 'refrigerator', 'tv-screen', 'table']
        ...
        # self.basepath = '/home/nel/gsarch/replica_orbslam_noisy_pose's
        self.homepath = f'/home/nel/gsarch/replica_traj_{self.include_classes[0]}'
        ...
        if not os.path.exists(self.homepath):
            ...
            if val == 'y':
                ...
                os.mkdir(self.homepath)
                # filelist = glob.glob(os.path.join(self.basepath, "*"))
                # for f in filelist:
                #     os.remove(f)
            else:
                print("ENDING")
    ...
    def make_cfg(self, settings):
        ...
        for sensor_uuid, sensor_params in sensors.items():
            if settings[sensor_uuid]:
                ...
                if sensor_uuid == 'depth_sensor_noisy' and self.do_depth_noise:
                    # add depth noise
    ...
    def save_datapoint(self, agent, observations, data_path, viewnum, mainobj_id, flat_view):
        ...
        if self.do_depth_noise:
            depth_noisy = observations["depth_sensor_noisy"]
        ...
        if self.do_depth_noise:
            save_data['depth_camX_noisy'] = depth_noisy
    ...
    def run(self):
        ...
        for obj in objects:
            ...
            if True:
                ...
                plt.plot(obj_center[:,2], obj_center[:,0], 'x', color = 'black')
                # plt.show()
                plt_name = 'samples/' + str(self.ep_idx) + '_' + str(plt_id) + '_' + 'samples.png'
            ...
            for b in range(pos_s_all_two.shape[0]):
                ...
                #         # pos_s[1] = y_height_fixed
                    ...
                    if self.do_orbslam or self.makevideo: 
                        im = observations["color_sensor"]
                        im = Image.fromarray(im, mode="RGBA")
                        im = im.convert("RGB")
                        str_id = '{0:03d}'.format(cnt)
                        im_name = str_id + '.png'
            ...
            if self.do_orbslam:
                ...
                associations_file = os.path.join(orbslam_dir, "Examples/RGB-D/associations/replica.txt")
                associations_executable = os.path.join(orbslam_dir, "Examples/RGB-D/associations/associate.py")
                rgb_file = os.path.join(orbslam_dir, "Examples/RGB-D/replica/rgb.txt")
                depth_file = os.path.join(orbslam_dir, "Examples/RGB-D/replica/depth.txt")
                ...
                for i in range(camXs_T_camX0_orb_output.shape[0]):
                    ...
                    # rot_quat = quaternion.from_rotation_matrix(r_origin_T_camX_orb)
                    rot_quat_vect = quaternion.as_rotation_vector(r_origin_T_camX_orb_quat)

                    # rot_quat_gt = r_origin_T_camX_quat.from_rotation_matrix(r_origin_T_camX_quat)
                    rot_quat_vect_gt = quaternion.as_rotation_vector(r_origin_T_camX_quat)

                    rot_quat_vect[1] = rot_quat_vect_gt[1]
                    r_origin_T_camX_orb_quat = quaternion.from_rotation_vector(rot_quat_vect)
            ...
            if not self.do_orbslam:
                if len(episodes) >= self.num_views:
                    ...
                    interval = int(len(episodes)/self.num_views)
            ...
            # so we need to sample from episode number less than camXs_T_camX0_orb_output.shape[0]
                ...
                if num_valid_episodes >= self.num_views:
                    ...
                    # interval = np.arange(0, camXs_T_camX0_orb_output.shape[0], interval)
                    episodes = episodes[:num_valid_episodes]
                    interval = int(np.floor(camXs_T_camX0_orb_output.shape[0]/self.num_views))
                    episodes = episodes[::interval]
                    flat_obs = episodes #np.random.choice(episodes, self.num_views, replace=False)
                    viewnum = 0
                    for obs in flat_obs:
                        self.save_datapoint(self.agent, obs, data_path, viewnum, obj.id, True)
                        viewnum += 1
            ...
            if self.makevideo:
                ...
                # image_folder = self.orbslam_path + "/" + "rgb/"
                video_name = 'video/' + obj.category.name() + str(self.ep_idx) + '_' + str(plt_id) + '_frames.gif'

                # images = [img for img in os.listdir(image_folder) if img.endswith(".png")]
                # frame = cv2.imread(os.path.join(image_folder, images[0]))
                # height, width, layers = frame.shape

                # video = cv2.VideoWriter(video_name, 0, 1, (self.W,self.H))
                ...
                image_folder = self.orbslam_path + "/" + "rgb/*.png"

                fnames = sorted(glob.glob(image_folder))

                img, *imgs = [Image.open(f) for f in fnames[::interval]]
                img.save(fp=video_name, format='GIF', append_images=imgs,
                        save_all=True, duration=30, loop=0)
                
                video_name = 'video/' + obj.category.name() + str(self.ep_idx) + '_' + str(plt_id) + '_frames_all.gif'
