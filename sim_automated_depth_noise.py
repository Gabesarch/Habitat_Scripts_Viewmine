...
# 1       self.include_classes = ['chair', 'bed', 'toilet', 'sofa', 'indoor-plant', 'refrigerator', 'tv-screen', 'table']
    ...
    def run_episodes(self):
        ...
        for episode in range(self.num_episodes):
            ...
            for f in filelist:
                os.remove(f)
            filelist = glob.glob(os.path.join(self.orbslam_path,"depth", "*.png"))
            for f in filelist:
                os.remove(f)
    ...
    def save_datapoint(self, agent, observations, data_path, viewnum, mainobj_id, flat_view):
        ...
        # save depth image
        depth_img.save("depth_noise_gaussian.png")
        sys.exit()
    ...
    def run(self):
        ...
        for obj in objects:
            ...
            if self.do_orbslam:
                ...
                try:
                    output = subprocess.run([executable, vocabulary_file, settings_file, data_dir, associations_file], capture_output=True, text=True, check=True, timeout=800)
                except:
                ...
                for i in range(camXs_T_camX0_orb_output.shape[0]):
                    ...
                    r_origin_T_camX_orb, t_origin_T_camX_orb = self.split_rt_single(origin_T_camX_4x4)
                ...
                # plt.legend()
                plt_name = 'maps/' + str(self.ep_idx) + '_' + str(id) + '_' + 'map.png'
