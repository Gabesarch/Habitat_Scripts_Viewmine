...
class AutomatedMultiview():
    def __init__(self):
        ...
        self.num_objects_per_episode = 1 #30 #30
        # Initialize maskRCNN
        cfg_det = get_cfg()
    ...
    def run_episodes(self):
        ...
        for episode in range(self.num_episodes):
            ...
            self.basepath = f"/home/nel/gsarch/data/test/{mapname}_{episode}"
    ...
    def run(self):
        ...
        if True: # GROUND TRUTH FOR TESTING AND PLOTTING
            ...
            for obj in objects:
                ...
                obj_names.append(obj.category.name())
        ...
        while gif_success < 5: #self.num_objects_per_episode:
            # object_centers = self.get_midpoint_obj_conf()
            obj_idx += 1

            if obj_idx >= object_centers.shape[0]:
                break

            #Calculate distance to object center
            obj_center = object_centers[obj_idx]

            # obj_id = int(obj_ids[obj_idx].split('_')[1])
            
            print(obj_center)
            obj_center = np.expand_dims(obj_center, axis=0)

            print("OBJECT CENTER: ", obj_center)
            print("object name: ", obj_names[obj_idx])
            ...
            spawns_per_bin = 1 #int(self.num_views / num_valid_bins) + 1
            ...
            if True:
                ...
                for idx_s in range(num_iters):
                    ...
                    if cnt==0:
                        agent_state = habitat_sim.AgentState()
                        agent_state.position = pos_s
                        quat_yaw = quat_from_angle_axis(0, np.array([0, 1.0, 0]))
                        ...
                        if not obj_masks:
                            ...
                            obj_select_ind = random.choice(range(len(obj_ids)))
                            obj_mask_focus = obj_masks[obj_select_ind] #random.choice(obj_masks)
                            obj_id_focus = obj_ids[obj_select_ind]
                        ...
                        # spawns_per_bin = 2 #int(self.num_views / num_valid_bins) + 1
                        # print(f'spawns_per_bin: {spawns_per_bin}')
                        ...
                        im2 = Image.fromarray(seg_im, mode="RGB")
                        size=(256,256)
                        #resize image
                        im2 = im2.resize(size)
                        d1 = ImageDraw.Draw(im2)
                        myFont = ImageFont.truetype('E:/PythonPillow/Fonts/FreeMono.ttf', 20)
                        d1.text((0, 0), "*Run Object Detector", font=myFont, fill =(255, 0, 0))
                        for h in range(60):
                            self.gif_images.append(im2)

                        outputs_select = outputs['instances'][obj_id_focus]

                        v = Visualizer(im[:, :, ::-1], MetadataCatalog.get(self.cfg_det.DATASETS.TRAIN[0]), scale=1.2)
                        out = v.draw_instance_predictions(outputs_select.to("cpu"))
                        seg_im = out.get_image()
                        ...
                        d1 = ImageDraw.Draw(im2)
                        myFont = ImageFont.truetype('E:/PythonPillow/Fonts/FreeMono.ttf', 20)
                        d1.text((0, 0), "*Target Detection", font=myFont, fill =(255, 0, 0))
                        for h in range(60):
                            self.gif_images.append(im2)
                    ...
                    for action_path in actions_path:
                        if action_path is None:
                            continue
                        observations = self.sim.step(action_path)
                        im = observations["color_sensor"]
                        im = Image.fromarray(im, mode="RGBA")
                        self.gif_images.append(im)
                    ...
                    if turn_rights > turn_lefts:
                        turn_rights = turn_rights - turn_lefts
                        # print("1")
                        # print("turn left: ", turn_rights)
                        # print(".")
                        if turn_rights == 0:
                            pass
                        else: 
                            for turn in range(turn_rights):
                                # st()
                                movement_actual -= 1
                                # observations = self.sim.step("turn_left")
                                # im = observations["color_sensor"]
                                # im = Image.fromarray(im, mode="RGBA")
                                # self.gif_images.append(im)
                                if self.verbose:
                    ...
                    elif turn_lefts > turn_rights:
                        turn_lefts = turn_lefts - turn_rights
                        # print("1")
                        # print("turn right: ", turn_lefts)
                        # print(".")
                        if turn_lefts == 0:
                            pass
                        else: 
                            for turn in range(turn_lefts):
                                # st()
                                movement_actual += 1
                                # observations = self.sim.step("turn_right")
                                # im = observations["color_sensor"]
                                # im = Image.fromarray(im, mode="RGBA")
                                # self.gif_images.append(im)
                                if self.verbose:
                    ...
                    # agent_state = self.agent.get_state()
                    agent_to_obj = np.squeeze(obj_center) - (self.agent.state.sensor_states['color_sensor'].position)# + np.array([0, 1.5, 0]))
                    ...
                    # agent_rot = np.degrees(quaternion.as_euler_angles(self.agent.state.sensor_states['color_sensor'].rotation)[1])
                    # turn_angle += agent_rot
                    if np.abs(turn_angle) > 180:
                        if turn_angle < 0:
                            turn_angle = turn_angle + 360
                        else:
                            turn_angle = turn_angle - 360

                    num_turns = np.abs(np.floor(turn_angle/self.rot_interval)).astype(int) # compute number of times to move head up or down by rot_interval  
                    ...
                    movement = "turn_left" if turn_angle>0 else "turn_right"

                    
                    # print("right", turn_rights)
                    # print("left", turn_lefts)
                    # print(num_turns)
                    # print(movement)
                    # print(".")

                    # # need to reverse the rotations taken
                    # # turn_rights2 = turn_rights
                    # # turn_rights = turn_lefts
                    # # turn_lefts = turn_rights2  
                    # turn_rights2 = turn_rights  
                    # turn_lefts2 = turn_lefts  
                    # movement2 = movement    

                    # if turn_rights > turn_lefts:
                    #     turn_rights = turn_rights - turn_lefts
                    #     if movement == "turn_right":
                    #         turn_rights = turn_rights - num_turns
                    #         if turn_rights < 0:
                    #             num_turns = np.abs(turn_rights)
                    #         else:
                    #             movement = "turn_left"
                    #             num_turns = turn_rights
                    ...
                    #         turn_lefts = turn_lefts - num_turns
                    #         if turn_lefts < 0:
                    #             num_turns = np.abs(turn_lefts)
                    #         else:
                    #             movement = "turn_right"
                    #             num_turns = turn_lefts
                    ...
                    # turn_rights = turn_rights2   
                    # turn_lefts = turn_lefts2  
                    # movement = movement2   

                    movement_opp = "turn_right" if movement=="turn_left" else "turn_left"
                    ...
                    # if movement=="turn_left":
                    #     print("2")
                    #     print("turn left: ", num_turns)
                    #     print(".")
                    # if movement=="turn_right":
                    #     print("2")
                    #     print("turn right: ", num_turns)
                    #     print(".")
                    if num_turns == 0:
                        pass
                    else: 
                    ...
                    # print("Movement acc=", movement_actual)

                    num_turns_eff = np.abs(movement_actual)
                    movement_eff = "turn_left" if movement_actual<0 else "turn_right"
                    rot_amount_deg = num_turns_eff * self.rot_interval
                    if rot_amount_deg > 180:
                        rot_amount_deg = 360 - rot_amount_deg
                        num_turns_eff = (rot_amount_deg//self.rot_interval).astype(int)
                        movement_eff = "turn_right" if movement_eff=="turn_left" else "turn_left"
                    ...
                    if num_turns == 0:
                        ...
                        for turn in range(num_turns_eff):
                            # if movement=="turn_left":
                            #     movement_actual -= 1
                            # if movement=="turn_right":
                            #     movement_actual += 1
                            # st()
                            observations = self.sim.step(movement_eff)
                            im = observations["color_sensor"]
                            im = Image.fromarray(im, mode="RGBA")
                            self.gif_images.append(im)
                            if self.verbose:
                    ...
                    # agent_rot = np.degrees(quaternion.as_euler_angles(self.agent.state.sensor_states['color_sensor'].rotation)[2])
                    
                    # turn_pitch += agent_rot
                    ...
                    #     self.agent.state.sensor_states[sensor].rotation = agent_state.rotation
                    #     self.agent.state.sensor_states[sensor].position = agent_state.position + np.array([0, 1.5, 0]) # ADDED IN UP TOP
                    # # if np.abs(turn_pitch) > 180:
                    # #     if turn_pitch < 0:
                    # #         turn_pitch = turn_pitch + 360
                    # #     else:
                    # #         turn_pitch = turn_pitch - 360
                    # turn_pitch = np.degrees(math.atan2(agent_to_obj[1], flat_dist_to_obj))
                    num_turns2 = np.abs(np.floor(turn_pitch/self.rot_interval)).astype(int) # compute number of times to move head up or down by rot_interval
                    # print("MOVING HEAD ", num_turns, " TIMES")
                    movement = "look_up" if turn_pitch>0 else "look_down"
                    movement_opp2 = "look_down" if turn_pitch>0 else "look_up"
                    ...
                    # agent_state = self.agent.get_state()
                    # print(np.degrees(quaternion.as_euler_angles(agent_state.rotation)))
                    ...
                    for h in range(10):
                        ...
                        myFont = ImageFont.truetype('E:/PythonPillow/Fonts/FreeMono.ttf', 20)
                        d1.text((0, 0), "*Get Observation", font=myFont, fill =(255, 0, 0))
                        self.gif_images.append(im)
                        # im = cv2.cvtColor(np.asarray(im), cv2.COLOR_RGB2BGR)
                    ...
                    # Rotate "head" of agent up or down based on calculated pitch angle to object to center it in view
                    movement_actual = 0
                    # if movement_opp=="turn_left":
                    #     print("3")
                    #     print("turn left: ", num_turns)
                    #     print(".")
                    # if movement_opp=="turn_right":
                    #     print("3")
                    #     print("turn right: ", num_turns)
                    #     print(".")
                    if num_turns == 0:
                        ...
                        for turn in range(num_turns):
                            ...
                            if movement_opp=="turn_right":
                                movement_actual += 1
                            # st()
                            # observations = self.sim.step(movement_opp)
                            # im = observations["color_sensor"]
                            # im = Image.fromarray(im, mode="RGBA")
                            # self.gif_images.append(im)
                            if self.verbose:
                    ...
                    if num_turns2 == 0:
                        ...
                        for turn in range(num_turns2):
                            # st()
                            observations = self.sim.step(movement_opp2)
                            im = observations["color_sensor"]
                            im = Image.fromarray(im, mode="RGBA")
                            self.gif_images.append(im)
                            if self.verbose:
            ...
            if self.gen_gif and len(self.gif_images) > 3:
                # im = self.gif_images[0]
                # ims = self.gif_images[1:]
                gif_cnt += 1
                self.gif_images[0].save('/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/gifs/' + os.path.split(self.basepath)[-1] + '_' + str(gif_cnt) + '.gif',
               save_all=True, append_images=self.gif_images[1:], optimize=False, duration=40, loop=0)
                # im.save('/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/gifs/' + os.path.split(self.basepath)[-1] + '_' + str(gif_cnt) + '.gif', save_all=True, append_images=ims)
                gif_success += 1
