...
class AutomatedMultiview():
    def __init__(self):   
        ...
        self.verbose = False
        self.save_imgs = True
        self.do_orbslam = True
        self.do_depth_noise = True
        self.min_orbslam_views = 150
        self.orbslam_path = '/home/nel/ORB_SLAM2/Examples/RGB-D/replica2'
        self.ORBSLAM_Cam_Traj = '/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/HabitatScripts_Second/CameraTrajectory.txt'
        self.ORBSLAM_Cam_Keypoints = '/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/HabitatScripts_Second/KeyFrameTrajectory.txt'
        # st()
    ...
    def run_episodes(self):
        self.ep_idx = 0
        if os.path.exists('/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/HabitatScripts_Second/maps'):
            dir_name = "/home/nel/gsarch/habitat/habitat-lab/HabitatScripts/HabitatScripts_Second/maps/"
        ...
        for episode in range(self.num_episodes):
            ...
            associations_file = "/home/nel/ORB_SLAM2/Examples/RGB-D/associations/replica2.txt"
            if os.path.exists(associations_file):
                deleted.append(['associations2'])
                os.remove(associations_file)
            if os.path.exists(self.orbslam_path + "/" + 'rgb2.txt'):
                deleted.append(['rgb2.txt'])
                os.remove(self.orbslam_path + "/" + 'rgb2.txt')
            if os.path.exists(self.orbslam_path + "/" + 'depth2.txt'):
                deleted.append(['depth2.txt'])
                os.remove(self.orbslam_path + "/" + 'depth2.txt')
    ...
    def run(self):
        ...
        for obj in objects:
            ...
            if self.do_orbslam:
                with open(self.orbslam_path + "/" + 'rgb2.txt', 'w') as f:
                    for item in texts_images:
                        f.write("%s\n" % item)
                
                with open(self.orbslam_path + "/" + 'depth2.txt', 'w') as f:
            ...
            if self.do_orbslam:
                ...
                data_dir = os.path.join(orbslam_dir, "Examples/RGB-D/replica2")
                associations_file = os.path.join(orbslam_dir, "Examples/RGB-D/associations/replica2.txt")
                associations_executable = os.path.join(orbslam_dir, "Examples/RGB-D/associations/associate.py")
                rgb_file = os.path.join(orbslam_dir, "Examples/RGB-D/replica2/rgb2.txt")
                depth_file = os.path.join(orbslam_dir, "Examples/RGB-D/replica2/depth2.txt")
            ...
            # so we need to sample from episode number less than camXs_T_camX0_orb_output.shape[0]
                ...
                if num_valid_episodes >= self.num_views:
                    ...
                    flat_obs = np.random.choice(episodes, self.num_views, replace=False)
                    print("NUMBER SAVED:", len(flat_obs))
                    viewnum = 0
                    for obs in flat_obs:
                        self.save_datapoint(self.agent, obs, data_path, viewnum, obj.id, True)
                        viewnum += 1
